<x-layouts.app>
    <div class="py-12 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 min-h-screen rounded-t-2xl">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <!-- Header Section -->
            <div class="mb-8">
                <h1 class="text-4xl font-bold text-white mb-2">Edit Team: {{ $team->name }}</h1>
                <p class="text-zinc-400 text-lg">Update team information</p>
            </div>
                    ...
                    <form method="POST" action="{{ route('admin.teams.update', $team) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" 
                                      ...
                                      rows="3" 
                                      class="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded-lg text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                                      placeholder="Enter team description (optional)">{{ old('description', $team->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
